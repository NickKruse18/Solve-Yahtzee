...
server = function(input, output) {
  ...
  observeEvent(input$ngs,{ TR$FR = !TR$FR }, ignoreNULL = F)
  observeEvent(input$nf1,{ TR$FR = !TR$FR }, ignoreNULL = F)
  observeEvent(input$nf2,{ TR$FR = !TR$FR }, ignoreNULL = F)
  observeEvent(input$nf3,{ TR$FR = !TR$FR }, ignoreNULL = F)
  observeEvent(input$nf4,{ TR$FR = !TR$FR }, ignoreNULL = F)
  observeEvent(input$nf5,{ TR$FR = !TR$FR }, ignoreNULL = F)
  observeEvent(input$rf1,{ TR$FR = !TR$FR }, ignoreNULL = F)
  ...
  output$calctime = renderText({
    update()
    return(paste0("Last Calculation took ",ct," seconds."))
  })
