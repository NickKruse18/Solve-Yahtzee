Scoring = function(throw){
  ...
  for(i in 1:sn){
    ...
    if(Rules[i,1]==3){
      d = diff(unique(throw));  con = 1
      for(j in d){
        if(j==1){ con = con + 1 } else { con = 1 }
        if(con>=Rules[i,3]){ s[i] = Rules[i,2];  break }
      }
    }
    if(Rules[i,1]==4){ s[i] = sum(throw) }
    if(Rules[i,1]==5){ 
      s[i] = (sum(throw==throw[1])==dice)*(Rules[i,2]+sum(throw)*Rules[i,3])
    }
  }
  return(s)
}
...
RuleNames = function(Rules){
  sn = nrow(Rules);  names = numeric(sn)
  for(i in 1:sn){
    if(Rules[i,1]==1){ names[i] = paste0(Rules[i,2],"'s") }
    if(Rules[i,1]==2){ 
      if(sum((Rules[i,3:4]-c(2,3))^2)==0){ names[i] = "Full House" }
      else if(sum((Rules[i,3:4]-c(3,2))^2)==0){ names[i] = "Full House" }
      else{
    ...
    if(Rules[i,1]==3){ names[i] = paste0("Straight with ", Rules[i,3]," numbers, ",Rules[i,2]," points") }
